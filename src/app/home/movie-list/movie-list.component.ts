...
export class MovieListComponent implements OnInit {
  ...
  getMoviesData() {
    ...
    this.homeService.getMethod(path).pipe(take(1)).subscribe((res: any) => {
      console.log(res)
  ...
  getSearch() {
    this.moviesData = [];
    this.homeService.searchPostMethod(this.searchValue).pipe(take(1)).subscribe((res: any) => {
      this.moviesData = res.results;
    });
  }
  sortLow(){
